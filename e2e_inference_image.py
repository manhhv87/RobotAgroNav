...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description='Semantic Segmentation and Semantic-Line Detection')
    parser.add_argument(
        '--input_dir', default='./inference/input/', help="Input images diretory")
    ...
    parser.add_argument('--align', default=False,
                        action='store_true', help='Enable alignment')
    parser.add_argument(
        '--palette',
        default='cityscapes',
        help='Color palette used for segmentation map')
    parser.add_argument(
        '--output_dir', default='./inference/output/', help='Output images directory')
...
segmentation_model = init_segmentor(
    segmentation_cfg, checkpoint=None, device='cuda:0')

segmentation_cfg.load_from = args.segmentation_model
checkpoint = load_checkpoint(
    segmentation_model, args.segmentation_model, map_location='cpu')
...
line_detection_model = line_detection_model.cuda(
    device=line_detection_cfg["TRAIN"]["GPU_ID"])
line_detection_checkpoint = torch.load(args.line_detection_model)


def main():
    if 'state_dict' in line_detection_checkpoint.keys():
        line_detection_model.load_state_dict(
            line_detection_checkpoint['state_dict'])
    else:
    ...
    for filename in os.listdir(args.input_dir):
        if filename.endswith(".jpg") or filename.endswith(".JPG") or filename.endswith(".png"):
            ...
            t = time.time()
            segmentation_result = inference_segmentor(
                segmentation_model, image)
            ntime += (time.time() - t)
            output_file = os.path.join(
                'inference/temp/', os.path.split(filename)[1])
...
def run_line_detection(test_loader, model, args):
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(bar):
            ...
            ) > line_detection_cfg['MODEL']['THRESHOLD']
            ...
            for i in range(len(b_points)):
                ...
                (x1, y1), (x2, y2) = get_boundary_point(
                    y1, x1, angle, size[0], size[1])
            ...
            np.save(os.path.join(args.output_dir, names[0].split(
                '/')[-1].split('.')[0]), np_data)

            if line_detection_cfg["MODEL"]["EDGE_ALIGN"] and args.align:
                for i in range(len(b_points)):
                    b_points[i] = edge_align(
                        b_points[i], names[0], size, division=5)
