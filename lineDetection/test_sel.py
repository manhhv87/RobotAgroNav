...
for filename in filenames:
    ...
    gt = [[int(float(l.rstrip().split(', ')[1])), int(float(l.rstrip().split(', ')[0])), int(
        float(l.rstrip().split(', ')[3])), int(float(l.rstrip().split(', ')[2]))] for l in gt_coords]
    for i in range(1, 100):
        ...
        if arg.align:
            tp, fp, fn = caculate_tp_fp_fn(
                pred_align.tolist(), gt.tolist(), thresh=i*0.01)
