...
for filename in filenames:
    ...
    if arg.align:
        pred_align = np.load(os.path.join(
            pred_path, filename.split('.')[0]+'_align.npy'))
    gt_txt = open(os.path.join(gt_path, filename.split('.')[0] + '.txt'))
    gt_coords = gt_txt.readlines()
    gt = []
    for i in range(int(gt_coords[0].split(' ')[0])):
        gt.append([int(float(gt_coords[0].split(' ')[i*4+2])), int(float(gt_coords[0].split(' ')[i*4+1])),
                  int(float(gt_coords[0].split(' ')[i*4+4])), int(float(gt_coords[0].split(' ')[i*4+3]))])
    for i in range(1, 100):
        ...
        if arg.align:
            tp, fp, fn = caculate_tp_fp_fn(
                pred_align.tolist(), gt, thresh=i*0.01)
...
f = 2 * total_recall * total_precision / \
    (total_recall + total_precision + 1e-6)
