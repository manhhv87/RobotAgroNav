...
class ResNet(nn.Module):
    ...
    def __init__(self, arch, block, layers, output_stride, BatchNorm, pretrained=True):
        ...
        if self.arch == 'resnet18':
            self.layer4 = self._make_layer(
                block, 512, layers[3], stride=strides[3], dilation=dilations[3], BatchNorm=BatchNorm)
        else:
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilation=1, BatchNorm=None):
        ...
        layers.append(block(self.inplanes, planes, stride, dilation, downsample,
                      BatchNorm, groups=self.groups, base_width=self.base_width))
        self.inplanes = planes * block.expansion
        for i in range(1, blocks):
            layers.append(block(self.inplanes, planes, dilation=dilation,
                          BatchNorm=BatchNorm, groups=self.groups, base_width=self.base_width))
    ...
    def forward(self, input):
        ...
        # Top-down
        p5 = self.toplayer(c5)
        p4 = nn.functional.upsample(
            p5, size=c4.size()[2:], mode='bilinear') + self.latlayer1(c4)
        p3 = nn.functional.upsample(
            p4, size=c3.size()[2:], mode='bilinear') + self.latlayer2(c3)
        p2 = nn.functional.upsample(
            p3, size=c2.size()[2:], mode='bilinear') + self.latlayer3(c2)
...
def FPN101(output_stride, BatchNorm=nn.BatchNorm2d, pretrained=True):
    ...
    """
    model = ResNet('resnet101', Bottleneck, [
                   3, 4, 23, 3], output_stride, BatchNorm, pretrained=pretrained)
    return model
...
def FPN50(output_stride, BatchNorm=nn.BatchNorm2d, pretrained=True):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet('resnet50', Bottleneck, [
                   3, 4, 6, 3], output_stride, BatchNorm, pretrained=pretrained)
    return model


def FPN18(output_stride, BatchNorm=nn.BatchNorm2d, pretrained=True):
    model = ResNet('resnet18', BasicBlock, [
                   2, 2, 2, 2], output_stride, BatchNorm, pretrained=pretrained)
    return model


def ResNext50_FPN(output_stride, BatchNorm=nn.BatchNorm2d, pretrained=True):
    model = ResNet('resnext50', Bottleneck, [
                   3, 4, 6, 3], output_stride, BatchNorm, pretrained=pretrained)
    return model
