...
class MobileNetV2(nn.Module):
    ...
    def _forward_impl(self, x):
        ...
        # Top-down
        p5 = self.toplayer(c5)
        p4 = nn.functional.upsample(p5, size=c4.size(
        )[2:], mode='bilinear', align_corners=True) + self.latlayer1(c4)
        p3 = nn.functional.upsample(p4, size=c3.size(
        )[2:], mode='bilinear', align_corners=True) + self.latlayer2(c3)
        p2 = nn.functional.upsample(p3, size=c2.size(
        )[2:], mode='bilinear', align_corners=True) + self.latlayer3(c2)
