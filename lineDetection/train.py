...
CONFIGS["OPTIMIZER"]["WEIGHT_DECAY"] = float(
...
def train(train_loader, model, optimizer, epoch, writer, args):
    ...
    for i, data in enumerate(bar):
        ...
        writer.add_scalar('train/hough_space_loss',
                          hough_space_loss.item(), epoch * iter_num + i)
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(bar):
            ...
            for i in range(1, 100):
                tp, fp, fn = caculate_tp_fp_fn(
                    b_points, gt_coords, thresh=i*0.01)
                total_tp[i-1] += tp
                total_fp[i-1] += fp
                total_fn[i-1] += fn
        ...
        f = 2 * total_recall * total_precision / \
            (total_recall + total_precision + 1e-8)
        ...
        if CONFIGS["MODEL"]["EDGE_ALIGN"]:
            ...
            logger.info(
                'Validation result (Aligned): ==== F-measure@0.95: %.5f' % f_align[95 - 1])
            writer.add_scalar('val/f-measure', acc.mean(), epoch)
...
class DayHourMinute(object):
    ...
    def __init__(self, seconds):
        ...
        self.minutes = int(
            (seconds - self.days * 86400 - self.hours * 3600) // 60)
