...
parser.add_argument('--model', default="./checkpoint/model_best.pth",
...
def main():
    ...
    model = Net(numAngle=CONFIGS["MODEL"]["NUMANGLE"], numRho=CONFIGS["MODEL"]
                ["NUMRHO"], backbone=CONFIGS["MODEL"]["BACKBONE"])
    ...
    if args.model:
        if isfile(args.model):
            ...
            logger.info(
                "=> no pretrained model found at '{}'".format(args.model))
    # dataloader
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(bar):
            ...
            visualize_save_path = os.path.join(
                CONFIGS["MISC"]["TMP"], 'visualize_test')
            os.makedirs(visualize_save_path, exist_ok=True)

            binary_kmap = key_points.squeeze().cpu(
            ).numpy() > CONFIGS['MODEL']['THRESHOLD']
            ...
            b_points = reverse_mapping(
                plist, numAngle=CONFIGS["MODEL"]["NUMANGLE"], numRho=CONFIGS["MODEL"]["NUMRHO"], size=(400, 400))
            scale_w = size[1] / 400
            scale_h = size[0] / 400
            ...
            cv2.imwrite(join(visualize_save_path,
                        names[0].split('/')[-1]), vis)
            np_data = np.array(b_points)
            np.save(join(visualize_save_path, names[0].split(
                '/')[-1].split('.')[0]), np_data)
            ...
            if CONFIGS["MODEL"]["EDGE_ALIGN"] and args.align:
                ...
                cv2.imwrite(join(visualize_save_path, names[0].split(
                    '/')[-1].split('.')[0]+'_align.png'), vis)
                np_data = np.array(b_points)
                np.save(join(visualize_save_path, names[0].split(
                    '/')[-1].split('.')[0]+'_align'), np_data)
            ntime += (time.time() - t)
