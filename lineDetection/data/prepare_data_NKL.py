...
for idx, label_file in enumerate(labels_files):
    ...
    if isfile(join(image_dir, filename+".jpg")):
        ...
        print("Warning: image %s doesnt exist!" %
    ...
    for argument in range(2):
        if argument == 0:
            ...
            with open(join(label_path, label_file)) as f:
                ...
                if int(nums) == 0:
                    print("Warning: image has no semantic line : %s" %
                          (filename))
            ...
            annotation = LineAnnotation(
                size=[args.fixsize, args.fixsize], lines=lines)
        else:
            im = cv2.flip(im, 1)
            ...
            with open(join(label_path, label_file)) as f:
                ...
                for i in range(int(data[0])):
                    y1, x1 = check(int(data[i*4+2]),
                                   W-1-int(data[i*4+1]), H, W)
                    y2, x2 = check(int(data[i*4+4]),
                                   W-1-int(data[i*4+3]), H, W)
                    line = Line([y1, x1, y2, x2])
                    line.rescale(scale_H, scale_W)
                    lines.append(line)
        ...
        for l in annotation.lines:
            theta, r = line2hough(l, numAngle=args.numangle,
                                  numRho=args.numrho, size=(newH, newW))
