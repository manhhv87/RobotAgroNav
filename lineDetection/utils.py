...
def visulize_mapping(b_points, size, filename):
    ...
    if os.path.split(os.sys.argv[0])[1] == 'end_to_end_inference.py':
        img = cv2.imread(os.path.join(os.path.split(
            os.path.dirname(__file__))[0], 'inference/temp', filename))
    else:
        img = cv2.imread(os.path.join(os.path.dirname(
            __file__), 'data/inference/input', filename))
    img = cv2.resize(img, size)
    for (y1, x1, y2, x2) in b_points:
        img = cv2.line(img, (x1, y1), (x2, y2), (255, 255, 0),
...
def edge_align(coords, filename, size, division=9):
    ...
    for d1 in range(-division, division+1):
        for d2 in range(-division, division+1):
            ...
            mask = np.zeros_like(hed)
            mask = cv2.line(mask, (nx1, ny1), (nx2, ny2),
                            color=1.0, thickness=3)
