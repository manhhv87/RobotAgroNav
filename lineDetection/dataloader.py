...
def get_loader(root_dir, label_file, batch_size, img_size=0, num_thread=4, pin=True, test=False, split='train'):
    if test is False:
        ...
        dataset = SemanLineDataset(os.path.join(os. getcwd(
        ...
        dataset = SemanLineDatasetTest(os.path.join(
            os. getcwd(), root_dir), label_file, transform=transform, t_transform=None)
    if test is False:
