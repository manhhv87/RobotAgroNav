...
class LineAnnotation(object):
    ...
    def oriental_mask(self):
        if self.__oriental_mask is None:
            ...
            for idx, l in enumerate(self.lines):
                mask1 = mask == (idx+1)
                orient = round((l.angle() + np.pi/2) / (np.pi /
                               self.divisions)) % self.divisions  # 0, 1, ..., 11
    ...
    def regression_label(self):
        if self.__regression_label is None:
            ...
            angle = np.zeros(self.size+[self.divisions]
                             ).reshape(-1, self.divisions)
            ...
            dy = indicesY - \
                np.tile(range(self.size[0]), (self.size[1], 1)).transpose()
            theta = orient[np.argmin(dist_pre_line, 0).reshape(-1)]  # [H*W]
            angle[:] = [-np.pi/2 + k*np.pi /
                        self.divisions for k in range(self.divisions)]
    ...
    def normed_offset(self):
        mask = self.__mask.astype(bool)
        bw_dist, _ = distance_transform_edt(
            np.logical_not(mask), return_indices=True)
        bw_dist[bw_dist == 0] = 1
